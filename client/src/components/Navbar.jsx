...
const Navbar = () => {
  ...
  return (
    <nav className="bg-gradient-to-r from-blue-700 via-blue-900 to-blue-700 shadow-lg sticky top-0 z-50">
          ...
          <div className="hidden md:flex items-center gap-2">
            {isAuthenticated ? (
              <>
                ...
                {/* Notifications */}
                <button className="relative p-2 text-white hover:text-white hover:bg-white/20 rounded-lg transition-all duration-300 ease-in-out">
                  <Bell size={20} className="transition-transform duration-200 hover:scale-110" />
                ...
                <div className="relative">
                  <button
                    ...
                    className="flex items-center gap-2 px-3 py-2 text-sm text-white hover:bg-white/20 rounded-lg transition-all duration-300 ease-in-out"
                  >
                    <div className="w-8 h-8 bg-white/20 rounded-full flex items-center justify-center font-semibold text-white transition-all duration-300">
                  ...
                  >
                    ...
                    <Link to="/dashboard" className="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" onClick={() => setIsProfileMenuOpen(false)}>
                      <LayoutDashboard size={16} /> Dashboard
                    </Link>
                    {(user?.role === 'admin' || user?.role === 'librarian') && (
                      <Link to={user.role === 'admin' ? '/admin' : '/librarian'} className="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" onClick={() => setIsProfileMenuOpen(false)}>
              ...
              <>
                ...
                <Link to="/register" className="bg-white text-blue-700 px-4 py-2 rounded-md text-sm font-medium hover:bg-white/90 transition-colors">Register</Link>
              </>
            )}
          </div>
        ...
        <div className={`md:hidden py-2 space-y-1 transform transition-all duration-300 ease-in-out ${isMobileMenuOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
          {isAuthenticated ? (
            <>
              ...
              <button onClick={handleLogout} className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-white hover:bg-white/20 rounded-md w-full">
